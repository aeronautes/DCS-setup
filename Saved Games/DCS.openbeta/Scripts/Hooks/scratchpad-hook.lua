local function loadScratchpad()
    ...
    local function saveUFC(path, content, override)
        if path == nil then
            return
        end
		...
		--log(l)
--		n, lathr,latmin,latsec,e,lonhr,lonmin,lonsec, _,alt = 
--			string.find(l, "%a%s+%d+°%d+.%d+', %a%s+%d+°%d+.%d'\n%wm, %d+ft")
--		log(lathr)
		file:write(l)
	    end
	    --file:write(content)
	...
	if string.find(path, "ufc.txt") then
	    saveUFC(path, content, true)
	    return
	end
